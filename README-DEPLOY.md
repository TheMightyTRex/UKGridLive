...
api/australia_current.php      JSON: latest Australia (NEM) snapshot (demand, spot price, generation mix) - needs openelectricity_api_key
api/australia_series.php       JSON: historical Australia (NEM) series for the sparklines - needs openelectricity_api_key
...
cron/fetch_openelectricity.php pulls Australia (NEM) demand/generation-by-fueltech/price from the Open Electricity API (optional - needs openelectricity_api_key)
...
| Every 10-15 minutes (optional) | `php /home/YOURUSERNAME/public_html/cron/fetch_openelectricity.php` |
...
You don't need to register for anything or generate an API key for Elexon, NESO, the Carbon Intensity API, EirGrid's Smart Grid Dashboard, or Open-Meteo - all five are public and keyless (confirmed while building this). EirGrid's Open Data Licence does require the attribution "Supported by EirGrid Group Data" wherever its data is shown - already included in `pages/ireland.html`'s footer, so there's nothing extra to do unless you customise that page. The EIA and Open Electricity are the two sources that do need a (free) key - see "About the data sources" below - `fetch_eia.php`/`fetch_openelectricity.php` each run and log `OK, 0 rows` without erroring if you skip either, so leaving them out doesn't break anything. Open Electricity's data is also the one source on this site licensed CC BY-NC 4.0 (non-commercial use, with attribution) rather than unrestricted or attribution-only - see `includes/config.php.example`'s comment above `openelectricity_api_key` before relying on it commercially.

## Step 6 - Verify it's working

Give the cron jobs 15–30 minutes to run at least once, then check either of these:

- Visit `https://yoursite.example/api/current.php`, `https://yoursite.example/api/ireland_current.php`, and (if you set `eia_api_key`/`openelectricity_api_key`) `https://yoursite.example/api/usa_current.php`/`https://yoursite.example/api/australia_current.php` directly in a browser. You should see JSON with `"live": true` and real numbers. `"live": false` means no data has landed yet for that page - check cron ran (see below).
- In phpMyAdmin, browse the `ingest_log` table. You should see rows with `status = OK` from each of the four core sources (`ELEXON`, `CARBON_INTENSITY`, `NESO`, `EIRGRID`), plus `EIA`/`OPENELECTRICITY` if you configured either (a row saying `OK, 0 rows, ..._api_key not set` is expected and fine if you didn't). A row with `status = ERROR` has a `message` column explaining what went wrong.

Once `api/current.php` shows live data, reload the homepage - the status strip (Time/Price/Emissions/Demand/Generation/Transfers) and the History charts will automatically switch from illustrative preview data to real data. Once `api/ireland_current.php` shows live data, the Ireland page's Emissions/Demand/Generation/GB interconnection stats and sparklines do the same (SEM price and the generation-mix breakdown stay illustrative regardless - see `pages/data-sources.html` for why). Once `api/usa_current.php`/`api/australia_current.php` shows live data, the USA/Australia pages' stats and sparklines switch from "-"/"Not available" placeholders to real numbers the same way - neither page has an illustrative fallback to switch away from, unlike the homepage/Ireland/ENTSO-E pages. There's no separate "go live" switch; it happens the moment there's real data to show.
...
The **homepage's status strip and History charts** (Demand/Generation/Price/Emissions/Transfers, all 8 time ranges) are fully wired to the real backend, with automatic fallback to illustrative data if the backend has nothing yet. The **Ireland page's status strip** (Emissions/Demand/Generation/GB interconnection) and its 3-hour sparklines are wired the same way, via `api/ireland_current.php`/`api/ireland_series.php`. The **USA page's Demand/Generation/Total interchange stats and 3-hour sparklines** are wired the same way too, via `api/usa_current.php`/`api/usa_series.php` - but only if you've filled in `eia_api_key`; skip that and those stats simply stay at "-"/"Not available" rather than showing a placeholder figure (unlike the homepage/Ireland/ENTSO-E pages, the USA page has no illustrative fallback at all). The **Australia page's Demand/Generation/Spot price stats and 3-hour sparklines** work the same way via `api/australia_current.php`/`api/australia_series.php` and `openelectricity_api_key`.
...
- **Open Electricity API** (`api.openelectricity.org.au`) - optional, powers the Australia page's demand/spot price/generation mix stats, covering AEMO's National Electricity Market (NEM) only, not Western Australia or the Northern Territory. Also needs a key: register free, instantly, at <https://platform.openelectricity.org.au/sign-up> and paste the key into `openelectricity_api_key` in `includes/config.php`. Unlike every other source above, its data is licensed <a href="https://creativecommons.org/licenses/by-nc/4.0/">CC BY-NC 4.0</a> - non-commercial use only, with attribution - so check that licence before relying on it if this install ever carries ads or a paid tier.
